...
public class DocsController(IWebHostEnvironment env) : ControllerBase
{
    ...
    {
        var path = Path.Combine(env.WebRootPath, "docs", "Nepsis-ProjectSpec.docx");
    ...
    public IActionResult ListDocs()
    {
        return Ok(new[]
        {
            new
            {
                name        = "Nepsis Project Specification",
                filename    = "Nepsis-ProjectSpec.docx",
