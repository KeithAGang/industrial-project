...
app.MapScalarApiReference(opt => opt.WithTitle("Nepsis API").WithTheme(ScalarTheme.Moon));

// Auto-migrate and seed on startup — retry with backoff until DB is ready
...
for (var attempt = 1; attempt <= MaxRetries; attempt++)
{
    try
    {
        ...
        await Nepsis.Api.Data.DbSeeder.SeedAsync(ctx);
    ...
    catch (Exception ex)
    {
        ...
        {
            Console.WriteLine($"[Startup] Database unavailable after {MaxRetries} attempts. Giving up: {ex.Message}");
            throw;
        }
        var delay = TimeSpan.FromSeconds(Math.Min(5 * attempt, 30));
        Console.WriteLine($"[Startup] DB not ready (attempt {attempt}/{MaxRetries}): {ex.Message}. Retrying in {delay.TotalSeconds}s…");
