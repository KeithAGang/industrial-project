...
function NotifIcon({ type }: { type: Notification['type'] }) {
  switch (type) {
    case 'Critical': return <WarningOctagon weight="duotone" className="text-[14px] text-red-400" />
    case 'High':     return <Warning        weight="duotone" className="text-[14px] text-orange-400" />
    case 'Medium':   return <Warning        weight="duotone" className="text-[14px] text-amber-400" />
    case 'Low':      return <Bell           weight="duotone" className="text-[14px] text-green-400" />
    default:         return <Info           weight="duotone" className="text-[14px] text-blue-400" />
  }
}
    ...
    <div className="p-4 space-y-4 animate-fade-in">
      ...
      {isLoading ? (
        <div className="space-y-2">
          {[...Array(8)].map((_, i) => <Skeleton key={i} className="h-16 w-full rounded" />)}
        </div>
      ) : !data?.items?.length ? (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          <Bell weight="duotone" className="text-4xl mb-2 opacity-25" />
          <p className="text-[12px]">{unreadOnly ? 'No unread notifications' : 'No notifications'}</p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 mb-0.5">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-[10px] text-muted-foreground font-mono">{timeAgo(n.createdAt)}</span>
                      {!n.isRead && (
                        <button
                          onClick={() => markReadMutation.mutate(n.id)}
                          className="h-1.5 w-1.5 rounded-full bg-primary shrink-0 hover:bg-primary/50 transition-colors"
                          title="Mark as read"
                        />
                      )}
                    </div>
                  ...
                  <p className="text-[10px] text-muted-foreground/60 mt-1 font-mono">{n.solutionName}</p>
                </div>
              </div>
            </div>
          ))}
